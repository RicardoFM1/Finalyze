...
export const useAuthStore = defineStore('auth', () => {
    ...
    function hasFeature(featureSlug) {
        ...
        // Bloqueio explÃ­cito de Admin para colaboradores
    ...
    async function verifyCode(email, codigo) {
        try {
            ...
            if (!response.ok) throw new Error(data.message || 'Falha na verificaÃ§Ã£o');
    ...
    async function resendCode(email) {
        try {
            ...
            if (!response.ok) throw new Error(data.message || 'Falha ao reenviar cÃ³digo');
    ...
    function setLanguage(lang) {
        ...
        if (typeof window !== 'undefined') {
            ...
            const msg = lang === 'pt' ? 'Idioma alterado para PortuguÃªs' : 'Language changed to English';
    ...
    async function forgotPassword(email) {
        try {
            ...
            const data = await response.json();
            if (!response.ok) throw new Error(data.message || 'Falha ao solicitar recuperaÃ§Ã£o');
            return true;
        } catch (error) {
            console.error(error);
            throw error;
