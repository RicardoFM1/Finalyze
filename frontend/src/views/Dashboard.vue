<template>
  <v-container class="dashboard-wrapper">
    <v-row class="mb-8 animate-enter">
        <v-col cols="12">
    ...
    <v-row class="mb-8">
        ...
        <template v-else>
            <v-col cols="12" md="4" class="animate-enter delay-100">
                <v-card class="summary-card glass-card receita-gradient rounded-xl" elevation="6">
                    <v-card-item>
                        ...
                        <div class="text-h4 font-weight-bold">R$ {{ formatCurrency(resumo.receita) }}</div>
                        <div class="text-caption mt-2 opacity-80">Sua receita total</div>
                    </v-card-item>
                </v-card>
            </v-col>
            <v-col cols="12" md="4" class="animate-enter delay-200">
                <v-card class="summary-card glass-card despesa-gradient rounded-xl" elevation="6">
                    <v-card-item>
                        ...
                        <div class="text-h4 font-weight-bold">R$ {{ formatCurrency(resumo.despesa) }}</div>
                        <div class="text-caption mt-2 opacity-80">Sua despesa total</div>
                    </v-card-item>
                </v-card>
            </v-col>
            <v-col cols="12" md="4" class="animate-enter delay-300">
                <v-card class="summary-card glass-card saldo-gradient rounded-xl" elevation="6">
                    <v-card-item>
                        ...
                        <div class="text-h4 font-weight-bold">R$ {{ formatCurrency(resumo.saldo) }}</div>
    ...
    <v-row>
        <v-col cols="12" md="8" class="animate-enter delay-400">
            <v-card class="rounded-xl recent-activity overflow-hidden hover-card" elevation="4">
                ...
                <v-list lines="two" class="pa-2">
                    <v-list-item v-for="item in resumo.atividades_recentes" :key="item.id" class="rounded-lg mb-1"
                        ...
                        <template v-slot:prepend>
                            <v-avatar :color="item.tipo === 'receita' ? 'success-lighten-4' : 'error-lighten-4'" rounded="lg">
                                <v-icon :icon="item.tipo === 'receita' ? 'mdi-cash-plus' : 'mdi-cash-minus'"
                                    :color="item.tipo === 'receita' ? 'success' : 'error'"></v-icon>
                            </v-avatar>
                        </template>
        ...
        <v-col cols="12" md="4" class="animate-enter delay-400">
            <v-card class="rounded-xl mb-6 quick-actions hover-card" elevation="4" color="grey-lighten-4">
                <v-card-title class="font-weight-bold pa-4 pb-0">Ações Rápidas</v-card-title>
                <v-card-text class="pa-4">
                    <v-btn block color="primary" size="large" class="mb-4 rounded-lg btn-responsive" prepend-icon="mdi-plus"
                        @click="dialog = true" elevation="2">Novo Lançamento</v-btn>

                    <v-row dense class="mx-0">
                        <v-col cols="6" class="px-2">
                            <v-btn block variant="outlined" color="primary" class="rounded-lg btn-responsive"
                                prepend-icon="mdi-chart-bar" :to="{ name: 'Reports' }">
                                Relatórios
                            </v-btn>
                        </v-col>
                        <v-col cols="6" class="px-2">
                            <v-btn block variant="outlined" color="primary" class="rounded-lg btn-responsive"
                                prepend-icon="mdi-flag" :to="{ name: 'Metas' }">
                                Metas
                            </v-btn>
                        </v-col>
                    </v-row>
            ...
            <v-card v-if="metasSummary.length" class="rounded-xl metas-preview hover-card" elevation="4">
                ...
                <v-card-text class="pa-4">
                    <div v-for="meta in metasSummary" :key="meta.id" class="mb-4">
                        ...
                        <v-progress-linear :model-value="calculatePercentage(meta)" :color="meta.cor || 'primary'"
                            height="6" rounded></v-progress-linear>
                    </div>
                    <v-btn block variant="text" size="small" color="primary" :to="{ name: 'Metas' }">Ver todas as
                        metas</v-btn>
                </v-card-text>
            </v-card>
        </v-col>
...
const categorias = [
  ...
  { title: 'Salário', icon: 'mdi-cash-multiple' },
  { title: 'Investimentos', icon: 'mdi-chart-line' },
  { title: 'Renda Extra', icon: 'mdi-briefcase-plus' },
  ...
  { title: 'Gás', icon: 'mdi-fire' },
  { title: 'Manutenção Casa', icon: 'mdi-tools' },
  { title: 'Móveis/Decoração', icon: 'mdi-lamp' },

  
  { title: 'Mercado', icon: 'mdi-cart' },
  { title: 'Restaurante', icon: 'mdi-silverware-fork-knife' },
  { title: 'Lanches/Delivery', icon: 'mdi-food' },
  { title: 'Bar/Happy Hour', icon: 'mdi-glass-mug-variant' },

  
  { title: 'Combustível', icon: 'mdi-gas-station' },
  { title: 'Uber/Táxi', icon: 'mdi-taxi' },
  ...
  { title: 'Médico/Dentista', icon: 'mdi-doctor' },
  { title: 'Academia/Esportes', icon: 'mdi-dumbbell' },
  { title: 'Cuidados Pessoais', icon: 'mdi-account-star' }, 

  
  { title: 'Educação', icon: 'mdi-school' },
  { title: 'Cursos', icon: 'mdi-notebook-outline' },
  { title: 'Livros', icon: 'mdi-book-open-variant' },

  
  { title: 'Lazer', icon: 'mdi-beach' },
  { title: 'Viagem', icon: 'mdi-airplane' },
  { title: 'Streaming/Assinaturas', icon: 'mdi-youtube-subscription' }, 
  { title: 'Jogos', icon: 'mdi-controller' },
  { title: 'Cinema/Teatro', icon: 'mdi-movie-open' },

  
  { title: 'Roupas', icon: 'mdi-tshirt-crew' },
  { title: 'Eletrônicos', icon: 'mdi-laptop' },
  ...
  { title: 'Pets', icon: 'mdi-paw' },
  { title: 'Doações', icon: 'mdi-hand-heart' },
  { title: 'Crianças', icon: 'mdi-baby-carriage' },
...
.summary-card .opacity-80 { opacity: 0.85; }
.recent-activity :deep(.v-list) { background: transparent; }
.v-btn-toggle .v-btn { text-transform: none; letter-spacing: normal; }
