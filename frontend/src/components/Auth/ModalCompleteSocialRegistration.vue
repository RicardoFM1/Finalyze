  ...
  <ModalBase :model-value="modelValue" persistent maxWidth="500px" :title="$t('auth.complete_registration.title') || 'Completar Cadastro'">
    <div class="pa-6">
      <p class="text-body-2 text-medium-emphasis mb-6">
        {{ $t('auth.complete_registration.desc') || 'Para continuar sua seguranca, precisamos de mais alguns dados.' }}
      </p>
            ...
            >
              <template v-slot:label>
                <span class="text-caption">
                  ...
                  <span class="text-primary font-weight-bold cursor-pointer hover-underline" @click.stop="">{{ $t('auth.terms_and_conditions') || 'termos e condicoes' }}</span>
                </span>
              </template>
            </v-checkbox>
          </v-col>
...
const validateAge = (v) => {
  ...
  return age >= 18 || 'Voce deve ter pelo menos 18 anos'
}
